    ...
    companion object {
        ...
        private fun createPartnerInfo(serviceId: String): PartnerInfo? {
            return try {
                ...
                Log.d("PartnerInfoHolder", "Bundle configurado com PARTNER_SERVICE_TYPE: INAPP_PAYMENT, EXTRA_ISSUER_NAME: Builders Wallet")
                
                // Criar PartnerInfo diretamente!
                PartnerInfo(serviceId, bundle)
            } catch (t: Throwable) {
                Log.e("PartnerInfoHolder", "Falha ao criar PartnerInfo: ${t.message}", t)
                // Fallback: tentar criar apenas com serviceId
                try {
                    PartnerInfo(serviceId)
                } catch (fallbackError: Throwable) {
                    Log.e("PartnerInfoHolder", "Falha no fallback: ${fallbackError.message}", fallbackError)
                    null
                }
            }
        }
